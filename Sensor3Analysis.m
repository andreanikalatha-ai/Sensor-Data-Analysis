...
ch3_plot_baseline_corrected = ch3 - ch3_avg_baseline;%Subtract baseline
new_baseline_avg = mean(ch3_plot_baseline_corrected(baseline_idx));
...
ch3_step_fit = zeros(size(t)); % Initialize the new step-fit vector
...
for k = 1:length(conc) 
    ...
    data_for_avg = ch3_plot_baseline_corrected(avg_idx); % Use the original, unsmoothed data
    ...
    ch3_step_fit(plateau_idx) = step_avg;
...
t_plot = t(plot_idx);
ch3_plot_baseline_corrected=ch3_plot_baseline_corrected(plot_idx);
ch3_step_fit=ch3_step_fit(plot_idx);

% Plot the ORIGINAL (baseline-corrected) data
plot(t_plot, ch3_plot_baseline_corrected, 'b-', 'DisplayName', 'Original Data (Noisy Steps)', 'LineWidth', 0.5); 
hold on;

% Plot the IDEALIZED FLAT-LINE Step Data
plot(t_plot, ch3_step_fit, 'r-', 'DisplayName', 'Idealized Flat-Line Steps', 'LineWidth', 2.5); 
...
xlabel('Time (s)');
ylabel('Current (nA)');
title(['Sensor 3 Data and Step Fit']);
grid on
...
ylabel('Current (nA)');
title('Sensor 3 Callibration Curve');
grid on;
...
title('Sensor 3 Michaelis-Menten Curve');
grid on;
legend('Location', 'best');
hold off;
