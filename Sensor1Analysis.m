...
% --- 2. Gain correction ---
% Convert to seconds the time of gain change
...
gain_correction_time_s = H*3600 + M*60 + S; 
% Create the index for the gain correction period
gain_idx = t <= gain_correction_time_s;
% Apply the gain correction factor (0.1) to the current values
...
ch1_plot_baseline_corrected = ch1 - ch1_avg_baseline;%Subtract baseline
new_baseline_avg = mean(ch1_plot_baseline_corrected(baseline_idx));
...
ch1_step_fit = zeros(size(t)); % Initialize the new step-fit vector
...
for k = 1:length(conc) 
    ...
    data_for_avg = ch1_plot_baseline_corrected(avg_idx); % Use the original, unsmoothed data
    ...
    ch1_step_fit(plateau_idx) = step_avg;
...
t_plot = t(plot_idx);
ch1_plot_filtered=ch1_plot_baseline_corrected(plot_idx);
ch1_step_fit_filtered=ch1_step_fit(plot_idx);

% Plot the ORIGINAL (baseline-corrected) data
plot(t_plot, ch1_plot_filtered, 'b-', 'DisplayName', 'Original Data (Noisy Steps)', 'LineWidth', 0.5); 
hold on;

% Plot the IDEALIZED FLAT-LINE Step Data
plot(t_plot, ch1_step_fit_filtered, 'r-', 'DisplayName', 'Idealized Flat-Line Steps', 'LineWidth', 2.5); 
...
xlabel('Time (s)');
ylabel('Current (nA)');
title(['Sensor 1 Data and Step Fit']);
grid on
...
noise_data = ch1_plot_baseline_corrected(baseline_idx);
...
ylabel('Current (nA)');
title('Sensor 1 Callibration Curve');
grid on;
...
title('Sensor 1 Michaelis-Menten Curve');
grid on;
legend('Location', 'best');
hold off;
